...
private[spark] object Select {
  ...
  def requestCSV(location: Option[String], params: Map[String, String],
    ...
    new SelectObjectContentRequest() { request =>
      ...
      csvInput.withQuoteCharacter(params.getOrElse("quote", "\""))
      csvInput.withQuoteEscapeCharacter(params.getOrElse("escape", "\\"))
      csvInput.withComments(params.getOrElse("comment", "#"))
